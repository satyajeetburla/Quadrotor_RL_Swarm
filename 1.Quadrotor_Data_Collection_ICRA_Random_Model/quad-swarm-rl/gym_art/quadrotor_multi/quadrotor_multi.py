...
class QuadrotorEnvMulti(gym.Env):
    ...
    def step(self, actions):
        obs, rewards, dones, infos = [], [], [], []
        # print("obs, rewards, dones, infos ",len(obs), len(rewards), dones, infos )
        # print("actions",actions)
        # Can collect action & next observation from here:
        ...
        #stae vector is the obs
        # print("obs, rewards, dones, infos ",obs, rewards, dones, infos )
        ...
        # print("drone_col_matrix, curr_drone_collisions, distance_matrix",drone_col_matrix, curr_drone_collisions, distance_matrix)
        ...
        if self.use_obstacles:
            ...
            # print("obst_quad_col_matrixmatrix, quad_obst_pair", obst_quad_col_matrix, quad_obst_pair)
        ...
        # 0 if no crash with room & 1 if crash with room
        room_crash_fin = np.zeros(self.num_agents)
        room_crash_fin[room_crash_list1] = 1.0

        drone_safety = np.where((drone_col_matrix == 1.0) | (obst_col_data_fin == 1.0) | (room_crash_fin == 1.0), 1.0, 0)
        drone_col_met_dict = {
        ...
        if any(dones):
            ...
            if not self.saved_in_replay_buffer:
                ...
                approach_goal_list = []
                for i in range(len(infos)):
                    final_1s = (1.0 / self.envs[0].dt) * np.mean(self.distance_to_goal[i, int(-1 * self.control_freq):])
                ...
                self.base_success_rate_dict[scenario_name].append(float(base_success_flag))
        ...
        # return obs, rewards, dones, infos, drone_safety, drone_col_met_dict
